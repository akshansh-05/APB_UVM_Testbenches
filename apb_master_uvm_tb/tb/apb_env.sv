...
//     ├── slv_agent   (apb_slv_agent)  — ACTIVE agent: emulates slave responses
//     │   ├── drv     (apb_slv_driver) — Responds with PREADY, PRDATA
//     │   ├── sqr     (apb_sequencer)  — Sequencer for future extensibility
//     │   └── mon     (apb_monitor)    — Captures actual bus transactions
...
//     slv_agent.mon.ap  ──→  scoreboard.act_port  (actual transactions)
//
...
class apb_env extends uvm_env;
  ...
  apb_slv_agent    slv_agent;     // Active agent: emulates slave on APB bus
  ...
  //
  // Connection 2: slv_agent.mon.ap → scoreboard.act_port
  //   The bus monitor (inside the slave agent) captures what ACTUALLY HAPPENED
  //   on the APB bus and sends it to the scoreboard for comparison against expected.
  // ---------------------------------------------------------------------------
  function void connect_phase(uvm_phase phase);
    super.connect_phase(phase);
    sys_agent.mon.ap.connect(scoreboard.exp_port);      // Expected transactions
    slv_agent.mon.ap.connect(scoreboard.act_port);      // Actual transactions
  endfunction
